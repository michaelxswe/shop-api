import asyncpg
from databases.item_database import ItemDatabase
from fastapi import Depends
from models import ItemModel, ItemRatingModel


class ItemService:
    def __init__(self, item_database: ItemDatabase = Depends()):
        self.item_database = item_database

    async def register_item(
        self, name: str, price: float, category: str, qty: int, db: asyncpg.Connection
    ) -> ItemModel:
        item = await self.item_database.register_item(name=name, price=price, category=category, qty=qty, db=db)
        return ItemModel(**dict(item))  # type: ignore

    async def get_item(self, item_id: int, db: asyncpg.Connection) -> ItemModel | None:
        item = await self.item_database.get_item(item_id=item_id, db=db)
    ...
    async def remove_item(self, item_id: int, db: asyncpg.Connection) -> None:
        await self.item_database.remove_item(item_id=item_id, db=db)

    async def get_all_items(self, db: asyncpg.Connection) -> list[ItemModel]:
        items = await self.item_database.get_all_items(db=db)
        item_models = []
    ...
    async def get_items_by_category(self, category: str, db: asyncpg.Connection) -> list[ItemModel]:
        items = await self.item_database.get_items_by_category(category=category, db=db)
        item_models = []
    ...
    async def decrease_qty(self, item_id: int, qty: int, db: asyncpg.Connection) -> None:
        await self.item_database.decrease_qty(item_id=item_id, qty=qty, db=db)

    async def increase_qty(self, item_id: int, qty: int, db: asyncpg.Connection) -> None:
        await self.item_database.increase_qty(item_id=item_id, qty=qty, db=db)

    async def register_item_rating(self, item_id: int, rating: int, db: asyncpg.Connection) -> None:
        await self.item_database.register_item_rating(item_id=item_id, rating=rating, db=db)

    async def get_item_ratings(self, item_id: int, db: asyncpg.Connection) -> list[ItemRatingModel]:
        item_rating_models = []

        item_ratings = await self.item_database.get_item_ratings(item_id=item_id, db=db)
